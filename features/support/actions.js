/**
 * Provides all the actions we'll use in our tests.
 */
...
const dataLayerExists = async (containerId) => {
  scope.dataLayer = await scope.currentPage.evaluateHandle(() => window.google_tag_manager[containerId].dataLayer);
  assert(Array.isArray(dataLayer));
};

const computeState = async (dataLayer) => {
  ...
  return dataLayer.reduce(reducer, []);
}
